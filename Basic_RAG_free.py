...
custom_prompt = PromptTemplate(
    ...
    template=(
        ...
        "Always copy the exact page tags like [page 136] from the context when citing."
...
qa_chain = RetrievalQA.from_chain_type(
    llm=llm,
    chain_type="stuff",
    #add mmr (to check for similarity in chunks)
    retriever=vectorstore.as_retriever(
        search_type="mmr",
        search_kwargs={"k":5, "fetch_k":40, "lambda_mult": 0.8} #lambda mult is a weight to define how important relevance is to diversity
    ),
    return_source_documents = True,
    chain_type_kwargs={"prompt": custom_prompt}
)

query = "What is the Junior Anti-Sex League"

#"When does Winston see Julia for the first time?"
